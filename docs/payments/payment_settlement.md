...
`944ecee`, submitting a batch to the provider is **approval-gated** via
`vs_workflow` (maker-checker). The engine gate is still keyed to a published
template, but as of `681456f` that template arrives **with the tenant's books**:
...
- leave money-out ungated on a newly provisioned entity - the
  `payments.payout_batch` template is published when the tenant's books are created,
  and the seeded stage never auto-skips, so a batch **parks for an approver** rather
  than paying itself out. The exception is an entity provisioned before `681456f`:
  it has no ladder until `seed_payout_approvals` is run, and until then a single
  `payments.payout.create` holder disburses directly (§8.1).
...
- **Workflow bridge (no migration):** `workflow_document_type =
  "payments.payout_batch"` (`models.py:190`); `school` property maps the
  entity-scoped batch to `entity.source_school` for the school-scoped approval
  engine, `branch` is always `None` (`models.py:238-251`).
...
- `amount` is `int(body.get("amount") or 0)` and must be `> 0`. `vendor` resolves
  **within the entity** by code or pk (`views.py:403-406,513-516`); a missing/foreign
...
- On `POST /payout-batches/` with `submit:true`, the direct dispatch is **skipped
  when a template exists** (`approval_required(batch)`, `views.py:538-541`) - the
...
→ PROCESSING on the PSP's accept, or FAILED on rejection (`services.py:387-470`).
...
### Payout batch - unseeded legacy path (no template)
Reachable only where no `payments.payout_batch` template resolves for the batch's
scope: an entity provisioned before `681456f` and never run through
`seed_payout_approvals`, or one whose ladder an administrator deliberately removed.
It is **not** the path a newly provisioned entity takes (§8.1).

`create_payout_batch` → **DRAFT** with PENDING children (`services.py:471-529`).
...
then `_recompute_batch_status` derives the aggregate (`services.py:533-589`).
Confirming children moves the batch COMPLETED / PARTIALLY_COMPLETED / FAILED.

### Payout batch - approval-gated (template published; the normal path)
```
...
1. Publish a `payments.payout_batch` template for the school (checker stage,
   `approver_source = ROLE` on `approver_role_key = payout-approver`, `SCHOOL`
   scope, `skip_if_no_approvers = False`, `tests.py:1400-1414`). Onboarding
   publishes the same shape for a new entity without anyone doing this by hand
   (§8.1); the test does it explicitly so the gate is unambiguous.
...
3. `POST /payout-batches/<id>/` (direct submit) → **400** "approval-gated; submit
   it for approval instead" (`views.py:628-633`). Batch stays DRAFT.
4. `POST /payout-batches/<id>/submit-for-approval/` → a `WorkflowInstance` is
   ...
   PSP dispatch** (`tests.py:1466-1475`).
5. The requester **cannot** approve their own batch (SoD enforced by the engine,
   `tests.py:1479-1494`). A distinct approver **holding the `payout-approver` role**
...
> Hardening pass (2026-07-12) closed items 2, 3, 4, 5, 6, 7. Item 1 was closed by
> `681456f`; what is left of it is an install-time step for entities that predate
> that commit, not an open gap for new ones.

1. ✅ **The payout gate is on from onboarding - a newly provisioned entity cannot
   disburse single-handed.** `approval_required(batch)` still asks whether a
   `payments.payout_batch` template resolves for the batch's scope
   (`views.py:538-541,623-633`), but publishing that template is no longer a command
   somebody had to remember: `provision_payout_approval` is registered against
   finance's entity provisioning from `VsPaymentsConfig.ready()`
   (`provisioning.py:15-30`, `apps.py:20-27`) and runs **inside the transaction that
   creates the entity** (`vs_finance/serializers.py:167-183`), so a failure takes the
   entity with it rather than leaving an ungated tenant behind.
   **Seeded blocked, not seeded open.** The ladder is one always-on APPROVAL stage
   with `skip_if_no_approvers=False`, ROLE-sourced on the `payout-approver` role,
   `advance_rule=ANY`, `on_rejection=TERMINAL`, `approver_scope=SCHOOL`
   (`approvals.py:36-85`), and `ensure_approver_role` creates that role with
   **nobody holding it** (`approvals.py:158-162`) - so the first batch submitted
   parks and asks for an approver instead of paying itself out. Re-seeding is
   non-destructive: a tenant that already has a ladder keeps whatever an
   administrator configured (`approvals.py:148-153`). The platform-wide fallback
   (`ensure_default_approval_templates`, `tenant=None`, `approvals.py:88-112`)
   upserts and is platform provisioning's to own; because its stage also never
   auto-skips, falling back to it parks a batch rather than paying it. The whole
   shape deliberately mirrors `vs_procurement.approvals`, so a tenant holds one
   mental model for who signs off on money leaving. Tests (`tests.py:2977-3051`):
   `PayoutOnboardingSeedTests.test_creating_books_publishes_the_payout_ladder`,
   `.test_the_seeded_stage_never_auto_skips`,
   `.test_the_approving_role_exists_and_nobody_holds_it`,
   `.test_provisioning_a_second_entity_changes_nothing`.
   **Residual - operational, pre-existing installs only.** The provisioner runs when
   an entity's books are *created*, and there is deliberately no blanket
   data-migration backfill, so an entity provisioned before `681456f` does not get a
   ladder retroactively. Those installs need a one-time `python manage.py
   seed_payout_approvals --platform --all-tenants`
   (`management/commands/seed_payout_approvals.py`); until it runs they take the
   unseeded path in §4 and a lone `payments.payout.create` holder can still
   `POST /payout-batches/<id>/` (or create-with-`submit:true`) unreviewed.
...
- `payments.payout_batch.submit` (SENSITIVE) - route a batch for approval
  (`views.py:651`).
- `payments.payout_batch.approve` / `.approve_high_value` (**CRITICAL**) - still
  seeded (`seed_payments_permissions.py:46-47`), but **not** what decides who may
  vote. vs_workflow retired its permission-keyed approver source (migration
  `0006_remove_workflowstage_approver_permission_key_and_more`) and the seeded stage
  resolves approvers from the `payout-approver` **role**; the vote endpoints are
  guarded by eligibility in the engine's service layer rather than by a payments key
  (`vs_workflow/views.py:475-485`). Nothing in the codebase reads these two keys
  today.
...
`batch.school = entity.source_school` (`models.py:238-247`); approvers are resolved
from that tenant's **active holders of the stage's role** (`payout-approver` as
seeded, `vs_workflow/services/approvers.py:78-97,108-119`), and a tenant with nobody
appointed resolves to no approvers, which parks the batch. The requester cannot
approve their own batch (engine-enforced separation of duties, `tests.py:1479-1494`).
...
`payments.payout.view_sensitive` (`serializers.py:80-83`); the movements feed masks
the same fields manually (`views.py:975-977`).

## 10. Code map

- `models.py:176-421` - `PayoutBatch` (+ workflow bridge), `PayoutInstruction`,
  `PaymentEvent`.
- `constants.py:67-139` - payout / batch statuses + terminal sets, audit actions.
- `services.py:387-688` - `initiate_payout`, `_dispatch_transfer`,
...
- `vs_finance/approvals.py` - `approval_required` (the template-keyed gate, shared
  with finance).
...
- `PayoutTests` (`tests.py:612-698`): initiate→PROCESSING; confirm→books
...
- `PayoutBatchTests` (`tests.py:724-798`): assemble without submit; submit
  ...
  PARTIALLY_COMPLETED.
- `PayoutBatchApprovalTests` (`tests.py:1282-1967`): gate off → direct submit works;
...
- `PayoutApprovalSeedingTests` (`tests.py:1971-2104`) and `PayoutOnboardingSeedTests`
  (`tests.py:2977-3051`): the seeded ladder's shape (one always-on ROLE stage that
  never auto-skips), the non-destructive re-seed, the command's arguments and
