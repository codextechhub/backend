...
import datetime  # Date arithmetic for depreciation schedule generation.

from django.db import transaction  # Keeps asset accounting mutations atomic.
from django.utils import timezone  # Supplies posted timestamps and default dates.
...
from .constants import (
    ...
    DepreciationMethod,  # Supported depreciation methods.
...
def _add_months(d: datetime.date, n: int) -> datetime.date:  # Advance a date by whole months.
    ...
    if month == 12:  # December's next month is January of next year.
        next_month_first = datetime.date(year + 1, 1, 1)  # First day after December.
    else:  # Any other month advances within the same year.
        next_month_first = datetime.date(year, month + 1, 1)  # First day of next month.
    last_day = (next_month_first - datetime.timedelta(days=1)).day  # Last valid day of target month.
    return datetime.date(year, month, min(d.day, last_day))  # Preserve day where possible, otherwise clamp.


def _asset_accounts(asset):  # Resolve the three GL accounts used by asset accounting.
...
def _straight_line_amounts(base: int, months: int) -> list[int]:  # Build exact straight-line monthly charges.
...
def _declining_balance_amounts(cost: int, salvage: int, months: int) -> list[int]:  # Build double-declining schedule.
...
def _acquire_asset_atomic(asset, *, credit_account=None, bank_account=None,
    ...
    entry = JournalEntry.objects.create(  # Create acquisition journal header.
...
def post_depreciation(asset, *, up_to_date, actor_user=None, allow_restricted=False):  # Public wrapper for per-asset depreciation.
...
def _post_depreciation_atomic(asset, *, up_to_date, actor_user=None, allow_restricted=False):  # Transactional per-asset depreciation.
    ...
    for row in due:  # Process each due schedule row.
        ...
        row.posted_at = timezone.now()  # Stamp posting time.
        row.save(update_fields=["is_posted", "journal", "posted_at", "updated_at"])  # Persist schedule posting state.
    ...
    if not asset.schedule.filter(is_posted=False).exists():  # No remaining schedule rows means asset is fully depreciated.
        asset.asset_status = AssetStatus.FULLY_DEPRECIATED  # Mark asset fully depreciated.
    asset.save(update_fields=["accumulated_depreciation", "asset_status", "updated_at"])  # Persist asset rollup.
...
def _due_depreciation(entity, up_to_date):  # Load all due unposted depreciation rows for an entity.
    """The unposted, due schedule rows across the entity's active assets."""
    from .models import DepreciationSchedule  # Depreciation schedule model.

    return list(  # Materialize due rows for preview/posting.
        DepreciationSchedule.objects  # Start from schedule rows.
...
def run_period_depreciation(entity, *, up_to_date, actor_user=None):  # Public wrapper for compound depreciation run.
...
def _run_period_depreciation_atomic(entity, *, up_to_date, actor_user=None):  # Transactional compound depreciation run.
    ...
    for row in charges:  # Assign each charge to its fiscal period.
        period = period_cache.get(row.depreciation_date)  # Reuse period lookup for same date.
    ...
    for period in sorted(groups, key=lambda p: p.start_date):  # Process period buckets in date order.
        ...
        for row in bucket["rows"]:  # Aggregate rows inside this period.
            ...
            expense[expense_acct] = expense.get(expense_acct, 0) + row.amount  # Group expense debit.
            accum[accum_acct] = accum.get(accum_acct, 0) + row.amount  # Group contra-asset credit.

        entry = JournalEntry.objects.create(  # Create compound depreciation journal header.
    ...
    for row in rows:  # Update all due rows, including zero rows.
        ...
        row.posted_at = timezone.now()  # Stamp posting time.
        row.save(update_fields=["is_posted", "journal", "posted_at", "updated_at"])  # Persist schedule row state.
        by_asset[row.asset] = by_asset.get(row.asset, 0) + row.amount  # Accumulate amount for asset rollup.
    for asset, amount in by_asset.items():  # Update each affected asset.
        ...
        if not asset.schedule.filter(is_posted=False).exists():  # No remaining schedule rows means fully depreciated.
            asset.asset_status = AssetStatus.FULLY_DEPRECIATED  # Mark asset fully depreciated.
        asset.save(update_fields=["accumulated_depreciation", "asset_status", "updated_at"])  # Persist asset rollup.
...
def _dispose_asset_atomic(asset, *, disposal_date, proceeds=0, bank_account=None,
    ...
    entry = JournalEntry.objects.create(  # Create disposal journal header.
    ...
    asset.save(update_fields=["disposal_journal", "disposal_date", "asset_status", "updated_at"])  # Persist disposal fields.
