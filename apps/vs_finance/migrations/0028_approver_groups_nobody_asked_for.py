"""Remove the finance approver groups a tenant never asked for, and the steps naming them.

Provisioning a tenant's books used to publish a full adjustment ladder and mint the
groups its steps named. A tenant that had said nothing about who approves a refund
ended up holding three groups it never created, with nobody in them, named after a
workflow nobody there had read, and a ladder whose thresholds were a guess.

Who approves a tenant's money is read from the organogram that tenant builds, so the
books now arrive holding an approval route per document type and nothing inside it.
This clears what the earlier behaviour left behind: the empty groups, and the steps
that named them. The routes themselves stay, empty, which is the intended end state. An
empty route is what makes a direct post a decision somebody confirms and is recorded
against, rather than a post nobody ever sees.
...
* a step a document actually ran through is evidence, so a group whose steps are not
  all deletable is left whole rather than half-cleared;
...
# ``vs_finance.constants``. A migration has to keep meaning what it meant on the day it
...
def remove_approver_groups_nobody_asked_for(apps, schema_editor):
    ...
    to erase a step a document ran through; both are checked up front so an in-use
    group is skipped whole rather than failing the deploy part-way.
    """
