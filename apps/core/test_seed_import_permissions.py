...
class SeedImportPermissionOwnershipTests(TestCase):
    """Import seeding owns definitions and role defaults, not permission groups."""

    def test_import_seeding_does_not_create_permission_groups(self):
        from vs_rbac.models import PermissionGroup

        _call("seed_actions")
        _call("seed_import_permissions")

        self.assertFalse(PermissionGroup.objects.exists())
