...
PERMISSIONS: list[tuple[str, str, str, str]] = [
    ...
    ("system.tenant_context.require",           "system", "require",       "Requires every tenant-scoped request to carry a valid institution context."),
    ("system.tenant_boundary.enforce",          "system", "enforce",       "Prevents cross-institution reads, writes, or reference leakage."),
    ...
    ("system.configuration.view",               "system", "view",          "Allows viewing platform and institution configuration values."),
    ...
    # ==========================================================================
    # TIER 1 — MODULE 1: INSTITUTION & BRANCH MANAGEMENT
    # ==========================================================================
    ("institution.institution.create",              "institution", "create",       "Create Institution alongside Subsidiaries."),
    ("institution.institution_slug.generate",       "institution", "generate",     "Auto-Generate Institution Slug."),
    ("institution.database_schema.provision",       "institution", "provision",    "Provision Institution Database Schema."),
    ("institution.localization.assign",             "institution", "assign",       "Assign Institution Region and Localization."),
    ("institution.lifecycle.configure",             "institution", "configure",    "Configure Institution Lifecycle States."),
    ("institution.module_access.toggle",            "institution", "toggle",       "Enable / Disable Modules Per Institution."),
    ("institution.primary_administrator.assign",    "institution", "assign",       "Assign Primary Institution Administrator."),
    ("institution.branding_assets.upload",          "institution", "upload",       "Upload Institution Branding Assets."),
    ("institution.metadata.update",                 "institution", "update",       "Update Institution Metadata."),
    ("institution.uniqueness.validate",             "institution", "validate",     "Validate Institution Uniqueness."),
    ("institution.data_isolation.enforce",          "institution", "enforce",      "Enforce Institution Data Isolation."),
    ("institution.access_state.suspend",            "institution", "suspend",      "Suspend Institution."),
    ("institution.access_state.reactivate",         "institution", "reactivate",   "Reactivate Suspended Institution."),
    ("institution.deletion_state.soft_delete",      "institution", "soft_delete",  "Soft Delete Institution."),
    ("institution.deletion_state.hard_delete",      "institution", "hard_delete",  "Hard Delete Institution."),
    ("institution.configuration.reset",             "institution", "reset",        "Reset Institution Configuration."),
    ("institution.health_status.view",              "institution", "view",         "View Institution Health Status."),
    ("institution.creation_audit_logs.track",       "institution", "track",        "Track Institution Creation Audit Logs."),
    ("institution.feature_flags.enforce",           "institution", "enforce",      "Enforce Institution-Level Feature Flags."),
    ("institution.provisioning.rollback",           "institution", "rollback",     "Rollback Failed Institution Provisioning."),
    ...
    ("vision_admin.institution_dashboard.view",                 "vision_admin", "view",          "View All Institutions Dashboard."),
    ("vision_admin.institution_configuration.edit",             "vision_admin", "edit",          "Edit Institution Configuration."),
    ("vision_admin.access_state.suspend_manage",                "vision_admin", "suspend_manage","Suspend / Unsuspend Institution."),
    ("vision_admin.data_configuration.reset",                   "vision_admin", "reset",         "Reset Institution Data or Configuration."),
    ("vision_admin.institution_provisioning_pipeline.monitor",  "vision_admin", "monitor",       "Monitor Institution Provisioning Pipeline."),
    ("vision_admin.failed_provisioning_steps.retry",            "vision_admin", "retry",         "Retry Failed Provisioning Steps."),
    ("vision_admin.institution_usage_metrics.view",             "vision_admin", "view",          "View Institution Usage Metrics."),
    ("vision_admin.institution_audit_logs.access",              "vision_admin", "access",        "Access Institution Audit Logs."),
    ("vision_admin.institution_user.impersonate",               "vision_admin", "impersonate",   "Impersonate Institution User."),
    ...
    ("vision_admin.feature_flags.toggle",                       "vision_admin", "toggle",        "Toggle Feature Flags Per Institution."),
    ...
    ("identity.institution_aware_login.enforce",    "identity", "enforce",  "Enforce Institution-Aware Login."),
    ...
    ("system_config.feature_flag_institution.enable",               "system_config", "enable",   "Enable Feature Flag Per Institution."),
    ("system_config.feature_flag_institution.disable",              "system_config", "disable",  "Disable Feature Flag Per Institution."),
    ...
    # ==========================================================================
    # TIER 2 — MODULE 8: INSTITUTION ONBOARDING
    # ==========================================================================
    ("onboarding.institution_onboarding.initiate",      "onboarding", "initiate", "Initiate Institution Onboarding."),
    ("onboarding.institution_metadata.capture",          "onboarding", "capture",  "Capture Institution Metadata."),
    ...
    # ==========================================================================
    ("analytics.dashboard.institution_overview_dashboard.view", "analytics.dashboard", "view",      "Display Institution Overview Dashboard."),
...
DEPENDENCIES: list[tuple[str, str, bool]] = [
    ...
    *[("system.tenant_boundary.enforce", d, True) for d, _ in _base()[:1]],
    ...
    # ------------------------------------------------------------------
    # MODULE 1: INSTITUTION & BRANCH MANAGEMENT
    # ------------------------------------------------------------------
    *[("institution.institution.create",             d, h) for d, h in _base_audit()],
    *[("institution.institution_slug.generate",      d, h) for d, h in _base()],
    *[("institution.database_schema.provision",      d, h) for d, h in _base_audit()],
    *[("institution.localization.assign",            d, h) for d, h in _base_audit()],
    *[("institution.lifecycle.configure",            d, h) for d, h in _base_audit()],
    *[("institution.module_access.toggle",           d, h) for d, h in _base_audit()],
    *[("institution.primary_administrator.assign",   d, h) for d, h in _base_audit()],
    *[("institution.branding_assets.upload",         d, h) for d, h in _base_audit()],
    *[("institution.metadata.update",                d, h) for d, h in _base_audit()],
    *[("institution.uniqueness.validate",            d, h) for d, h in _base()],
    *[("institution.data_isolation.enforce",         d, h) for d, h in _base()],
    *[("institution.access_state.suspend",           d, h) for d, h in _base_audit()],
    *[("institution.access_state.reactivate",        d, h) for d, h in _base_audit()],
    *[("institution.deletion_state.soft_delete",     d, h) for d, h in _base_audit()],
    *[("institution.deletion_state.hard_delete",     d, h) for d, h in _base_audit()],
    ("institution.deletion_state.hard_delete",       "institution.deletion_state.soft_delete", True),
    *[("institution.configuration.reset",            d, h) for d, h in _base_audit()],
    *[("institution.health_status.view",             d, h) for d, h in _base()],
    *[("institution.creation_audit_logs.track",      d, h) for d, h in _base()],
    *[("institution.feature_flags.enforce",          d, h) for d, h in _base()],
    *[("institution.provisioning.rollback",          d, h) for d, h in _base_audit()],

    # ------------------------------------------------------------------
    # MODULE 2: VISION ADMIN CONSOLE
    # ------------------------------------------------------------------
    *[("vision_admin.session.authenticate",                      d, h) for d, h in _base()],
    *[("vision_admin.institution_dashboard.view",               d, h) for d, h in _base()],
    *[("vision_admin.institution_configuration.edit",           d, h) for d, h in _base_audit()],
    ("vision_admin.institution_configuration.edit",              "vision_admin.institution_dashboard.view", False),
    ...
    *[("vision_admin.institution_provisioning_pipeline.monitor",d, h) for d, h in _base()],
    *[("vision_admin.failed_provisioning_steps.retry",          d, h) for d, h in _base_audit()],
    ("vision_admin.failed_provisioning_steps.retry",             "vision_admin.institution_provisioning_pipeline.monitor", False),
    *[("vision_admin.institution_usage_metrics.view",           d, h) for d, h in _base()],
    *[("vision_admin.institution_audit_logs.access",            d, h) for d, h in _base()],
    ("vision_admin.institution_audit_logs.access",               "system.audit.view", True),
    *[("vision_admin.institution_user.impersonate",             d, h) for d, h in _base_audit()],
    ("vision_admin.institution_user.impersonate",                "system.mfa.enforce", True),
    ("vision_admin.institution_user.impersonate",                "system.support.impersonation.impersonate", True),
    ...
    *[("identity.institution_aware_login.enforce",  d, h) for d, h in _base()],
    ...
    *[("system_config.feature_flag_institution.enable",               d, h) for d, h in _base_audit()],
    ("system_config.feature_flag_institution.enable",                  "system_config.feature_flag.create", True),
    *[("system_config.feature_flag_institution.disable",              d, h) for d, h in _base_audit()],
    ("system_config.feature_flag_institution.disable",                 "system_config.feature_flag.create", True),
    ...
    *[("communication.message_activity.audit",          d, h) for d, h in _base()],
    ...
    # ------------------------------------------------------------------
    # MODULE 8: INSTITUTION ONBOARDING
    # ------------------------------------------------------------------
    *[("onboarding.institution_onboarding.initiate",      d, h) for d, h in _base()],
    *[("onboarding.institution_metadata.capture",          d, h) for d, h in _base_audit()],
    ("onboarding.institution_metadata.capture",             "onboarding.institution_onboarding.initiate", True),
    ...
    *[("inventory.asset_records.archive",         d, h) for d, h in _base_audit()],
    ...
    # ------------------------------------------------------------------
    *[("analytics.dashboard.institution_overview_dashboard.view", d, h) for d, h in _base()],
    ...
    *[("analytics.dashboard.dashboard_metrics.filter",            d, h) for d, h in _base()],
    ("analytics.dashboard.dashboard_metrics.filter",               "analytics.dashboard.institution_overview_dashboard.view", False),
