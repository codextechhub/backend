...
# Periodic housekeeping, run only where a beat scheduler exists: the worker
# service starts with ``celery -A apps worker -B``. Eager-mode environments
# never execute these, and every task is idempotent, so a missed window is safe.
app.conf.beat_schedule = {
    ...
    # you look rather than something that tells you. These two do the telling.
    # Both are stateless, so a missed run costs a late notice and nothing else.
    ...
    # A payout batch is approved in a transaction and dispatched after it commits,
    # so a lost hand-off (broker down, worker restarted) leaves a payout nobody
    # sent. Each instruction claims itself, so a re-run cannot pay twice.
    ...
    # is the worst-case lateness a scheduled export can suffer, so it is
    # tighter than the nightly housekeeping below.
    "exports-dispatch-schedules": {
        "task": "vs_exports.dispatch_schedules",
        "schedule": crontab(minute="*/5"),
    },
    # Every half hour: close runs whose worker died mid-flight. A killed worker
    # leaves a run RUNNING for good, spinning on the Files screen and counting
    # against the tenant's three-in-flight limit until nobody can export at all.
    # Frequent because it is one indexed query on a run somebody is waiting for.
    "exports-sweep-abandoned-runs": {
        "task": "vs_exports.sweep_abandoned_runs",
        "schedule": crontab(minute="*/30"),
    },
    # Nightly: hard-delete storage for files past their 30-day availability and
    # mark them purged. Availability is derived at read time, so a missed night
    # delays reclaiming bytes without changing what anyone sees. Idempotent.
    ...
    # Daily: suspend schools PENDING for 90 days, then warn those at 76. Expiry
    # runs first, so nobody is warned about a deadline already passed. Measured
    # from Tenant.pending_since, so a reinstated school gets a fresh window.
