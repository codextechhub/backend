...
def _validate_dataset_specific_rules(import_batch) -> list[dict]:
    ...
    if dataset_type == "calendar_events":
        # Owned by the calendar, like the bank-statement rules below are owned
        # by finance. What a row means is the module's business, and the same
        # resolver runs again at execution so the two passes cannot disagree.
    ...
    if dataset_type == "guardians":
        ...
        return validate_guardians_import_batch(import_batch)
