...
seeder, or a load run before the app config was wired - because "there cannot be
any rows yet" is exactly the assumption that turns out to be wrong, and an
unbound row is not served at all.

Both models here carry their own tenant, so the lookup is the column rather than
a join. That matters more for a staff document than for most files: it is the
most sensitive payload this module serves, and an unbound row is refused rather
than served to the wrong school, which is the safe direction to fail in.
...
WAVE = "vs_staff.0002_bind_staff_files"
