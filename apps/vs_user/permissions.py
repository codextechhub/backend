...
class IsVisionStaff(BasePermission):
    """
    Vision staff accounts are school=NULL and user_type=VISION_STAFF.
...
class IsSchoolAdminOrVisionStaff(BasePermission):
    """
    Allows Vision staff OR school admins.
    """
    def has_permission(self, request, view):
        u = getattr(request, "user", None)
        if not (u and u.is_authenticated):
            return False
        return getattr(u, "user_type", None) in ("VISION_STAFF", "SCHOOL_ADMIN")
